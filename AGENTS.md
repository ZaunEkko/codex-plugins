...
插件包放在 `plugins/<plugin-name>/` 下。当前包含 `explanatory-output-style` 和 `commit-commands`。每个插件的清单位于 `.codex-plugin/plugin.json`；hook 放在 `hooks/`，skill 放在 `skills/<skill-name>/SKILL.md`。
...
`py_compile` 用于检查 Python 语法。直接运行 `session_start.py` 会输出 hook JSON payload，适合在修改注入上下文后确认内容。`python -m unittest discover -s tests` 运行所有插件测试。`codex plugin list` 用于确认 Codex 能发现已安装的市场插件。
...
新增插件时，既要测试市场发现能力，也要测试插件的主要 hook 或 skill 路径，并验证五种语言的文档入口完整。Skill 测试应覆盖隐式触发描述和显式 `$skill-name` 入口。如果 hook 需要 Codex 信任，应在 README 或插件文档中说明 `/hooks` 审查步骤。
