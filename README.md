...
| commit-commands | Plugin + Skills + UserPromptSubmit Hook | 为提交加入分段的模型与思考强度 attribution，通过 UTF-8 文件安全创建并验证 GitHub.com/GitHub Enterprise PR 正文，并 force 清理 gone 分支。 | [插件文档](docs/commit-commands/README.md) |
...
在每个用户 turn 开始时，插件 hook 会解析当前 Codex 模型与思考强度，并让 `$commit` 与 `$commit-push-pr` 为新提交写入固定标识和动态 `Model: <model> <effort>` 段落。`$commit-push-pr` 也支持发布 clean worktree 中已有的分支提交，通过 UTF-8 文件避开 Windows PowerShell 5.1 的管道乱码，并在创建后验证完整 PR 正文及结尾的 `Generated with [Codex](https://chatgpt.com/codex)`。安装或更新后需要在 `/hooks` 中重新审查并信任该 hook。
