...
skill 不會直接呼叫 `gh pr create`，而是將完整 PR 正文交給內建 `scripts/create_pr_with_attribution.py` wrapper。wrapper 會追加尾註、透過 `--body-file` 建立 PR、反讀正文，必要時修復一次，且只有最後一個非空白行確認為下列內容時才回傳 URL：
