...
skill 不會直接呼叫 `gh pr create`。它會先將完整 PR 正文儲存成 UTF-8 暫存 Markdown 檔，再透過內建 `scripts/create_pr_with_attribution.py` wrapper 自己的 `--body-file` 選項傳入，避免 Windows PowerShell 5.1 預設 ASCII 外部程式管線破壞非 ASCII 文字。wrapper 支援 GitHub.com 與 GitHub Enterprise PR URL，會追加尾註、透過 `gh pr create --body-file` 建立 PR、反讀完整正文，必要時修復一次，且只有完整正文一致時才回傳 URL：
