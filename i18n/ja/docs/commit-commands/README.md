...
ワークフロー手順は原版に従います。Codex 固有の差分には、ネイティブ skill 構造、明示的な意図による自動選択、`$skill-name` 呼び出し、および現在の Codex モデルを動的に渡す `UserPromptSubmit` hook が含まれます。
...
Codex は現在のモデル slug を各 command hook に直接渡します。プラグインは `UserPromptSubmit` ごとにモデル context を更新するため、モデル切り替え後の次の commit では新しいモデルが使われます。不安定な transcript は解析せず、設定からモデルを推測しません。hook が trust されていない、context がない、またはモデルが利用できない場合、skill は stage や commit の前に停止します。
