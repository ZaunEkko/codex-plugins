...
skill 不会直接调用 `gh pr create`。它先把完整 PR 正文保存为 UTF-8 临时 Markdown 文件，再通过内置 `scripts/create_pr_with_attribution.py` wrapper 自身的 `--body-file` 选项传入；Windows 下不会再经过 PowerShell 5.1 默认的 ASCII 外部程序管道。wrapper 支持 GitHub.com 与 GitHub Enterprise PR URL，会追加尾注、通过 `gh pr create --body-file` 创建 PR、反读完整正文，并在不一致时修复一次；只有完整正文验证通过才返回 PR URL：
