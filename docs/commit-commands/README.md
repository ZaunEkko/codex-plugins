...
Codex 会把当前模型 slug 直接传给 command hook，但当前 schema 不提供思考强度。插件会用当前 `turn_id` 与模型精确匹配 `transcript_path` 尾部的 `turn_context`，读取其中的 `effort`；匹配失败时，只有用户配置指向同一模型才回退到 `model_reasoning_effort`。解析器只提取 attribution 字段，不复制或保存 prompt 内容。由于 transcript 格式并非稳定接口，解析失败时会省略强度后缀；hook 未信任、模型上下文缺失或模型不可用时，skill 才会在暂存和提交前停止。
