...
skill 不会直接调用 `gh pr create`，而是把完整 PR 正文交给内置 `scripts/create_pr_with_attribution.py` wrapper。wrapper 会追加尾注、通过 `--body-file` 创建 PR、反读正文，并在缺失时修复一次；只有最后一个非空行验证通过才返回 PR URL：
